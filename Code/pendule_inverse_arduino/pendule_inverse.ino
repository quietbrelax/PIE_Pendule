...
constexpr float U_ALPHA = 0.1f;   // 0.0 = immobile, 1.0 = pas de lissage
...
constexpr float RAIL_HALF_LENGTH_MM = 150.0f;
...
//
constexpr float PENDULUM_LENGTH_M = 0.073f;   // longueur pendule (m) — à mesurer !

// Gains LQR (vitesse chariot en mm/s par unité d'état)
// θ en rad, θ̇ en rad/s, x en mm, ẋ en mm/s
constexpr float K_THETA  =  4500.0f;   // [mm/s / rad]
constexpr float K_DTHETA =  450.0f;   // [mm/s / (rad/s)]
constexpr float K_X      =    0.8f;   // [mm/s / mm]     (maintien de position)
constexpr float K_DX     =    2.5f;   // [mm/s / (mm/s)]
...
constexpr float CONTROL_ANGLE_RAD = 1.0f;   // ≈ 20°
